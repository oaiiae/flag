...
func ExampleGeneric_usage() {
	...
	fs.Var(values.GenericSlice(",", parse, format), "generics", "usage")
	fs.Var(values.GenericSliceVar(&[]pair{{"foo", "bar"}, {"quu", "quux"}}, ",", parse, format), "generics-var", "usage")

	ip := netip.MustParseAddr("1.2.3.4")
	fs.Var(values.Stringer(netip.ParseAddr), "ip", "usage")
	fs.Var(values.StringerVar(&ip, netip.ParseAddr), "ip-var", "usage")
	fs.Var(values.StringerSlice(",", netip.ParseAddr), "ips", "usage")
	fs.Var(values.StringerSliceVar(&[]netip.Addr{ip, ip}, ",", netip.ParseAddr), "ips-var", "usage")
	...
	fs.Var(values.StringerSlice(",", url.Parse), "urls", "usage")
	fs.Var(values.StringerSliceVar(&[]*url.URL{u, u}, ",", url.Parse), "urls-var", "usage")
	...
	fs.Var(values.TimeSlice(",", time.RFC3339), "times", "usage")
	fs.Var(values.TimeSliceVar(&[]time.Time{t, t}, ",", time.RFC3339), "times-var", "usage")
