...
func ExampleGeneric_usage() {
	...
	values.Generic(fs, "generic", "usage", nil, format)
	values.GenericVar(fs, &pair{"foo", "bar"}, "generic-var", "usage", nil, format)
	values.Generics(fs, "generics", "usage", nil, format)
	values.GenericsVar(fs, &[]pair{{"foo", "bar"}, {"quu", "quux"}}, "generics-var", "usage", nil, format)

	ip := netip.MustParseAddr("1.2.3.4")
	values.Stringer(fs, "ip", "usage", netip.ParseAddr)
	values.StringerVar(fs, &ip, "ip-var", "usage", netip.ParseAddr)
	values.Stringers(fs, "ips", "usage", netip.ParseAddr)
	values.StringersVar(fs, &[]netip.Addr{ip, ip}, "ips-var", "usage", netip.ParseAddr)

	u := &url.URL{Scheme: "foo", Path: "bar"}
	values.Stringer(fs, "url", "usage", url.Parse)
	values.StringerVar(fs, &u, "url-var", "usage", url.Parse)
	values.Stringers(fs, "urls", "usage", url.Parse)
	values.StringersVar(fs, &[]*url.URL{u, u}, "urls-var", "usage", url.Parse)
	...
	values.Times(fs, "times", "usage", time.RFC3339)
	values.TimesVar(fs, &[]time.Time{t, t}, "times-var", "usage", time.RFC3339)
